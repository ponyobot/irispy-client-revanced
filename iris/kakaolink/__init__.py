...
class IrisLink:
    ...
    def send_melon(
        ...
        kv = PyKV()
        config = kv.get("melon_kakaolink_config")
        if type(config) != dict or "app_key" not in config or "origin" not in config:
            raise ValueError("melon_kakaolink_config가 iris.db에 없습니다. 'iris melon_kakaolink on' 명령어로 설정해주세요.")
