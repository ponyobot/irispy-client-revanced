...
# 아래서부터 추가됨
def host_check(chat: ChatContext):
    return chat.sender.type == "HOST"

def is_host(func):
    def wrapper(*args, **kwargs):
        chat: ChatContext = args[0]
        return func(*args, **kwargs) if host_check(chat) else None
    return wrapper

def manager_check(chat: ChatContext):
    return chat.sender.type == "MANAGER"
